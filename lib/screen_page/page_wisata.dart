...
class _PageWisataState extends State<PageWisata> {
  ...
  Future<void> fetchWisata() async {
    final response = await http.get(Uri.parse('http://192.168.43.243/wisataDB/getWisata.php'));

    if (response.statusCode == 200) {
      final data = modulWisataFromJson(response.body);
      setState(() {
  ...
  Future<void> deleteWisata(String id) async {
    final response = await http.post(
      Uri.parse('http://192.168.43.243/wisataDB/deleteWisata.php'),
      body: {'id': id},
    );
          ...
          return Card(
            ...
            child: ListTile(
              leading: wisata.gambar.isNotEmpty
                  ? Image.network(
                'http://192.168.43.243/wisataDB/gambar/${wisata.gambar}',
                width: 50,
                height: 50,
                fit: BoxFit.cover,
              )
